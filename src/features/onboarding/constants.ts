...
export type OnboardingCurrency = {
  code: string;
  name: string;
};

export const ONBOARDING_CURRENCIES: OnboardingCurrency[] = [
...
const ONBOARDING_CURRENCY_CODES = new Set(ONBOARDING_CURRENCIES.map((item) => item.code));

const normalizeCurrencyCode = (value: string | null | undefined): string | null => {
  if (!value) {
    return null;
  }

  const normalized = value.trim().toUpperCase();
  return ONBOARDING_CURRENCY_CODES.has(normalized) ? normalized : null;
};

export const getDeviceCurrencyCode = (): string => {
  const currencyCode = normalizeCurrencyCode(Localization.getLocales()?.[0]?.currencyCode);
